...
public final class BalancedBracketSelectors {
	...
	public BalancedBracketSelectors(
		...
		final Matcher<List<String>>[] balanced = balancedBracketScopes.stream()
				.flatMap(selector -> {
					if ("*".equals(selector)) {
						this.allowAny = true;
						return Stream.empty();
				...
				.toArray(Matcher[]::new);
		this.balancedBracketScopes = balanced;

		@SuppressWarnings("unchecked")
		final Matcher<List<String>>[] unbalanced = unbalancedBracketScopes.stream()
				.flatMap(selector -> Matcher.createMatchers(selector).stream().map(m -> m.matcher))
				.toArray(Matcher[]::new);
		this.unbalancedBracketScopes = unbalanced;
	}
