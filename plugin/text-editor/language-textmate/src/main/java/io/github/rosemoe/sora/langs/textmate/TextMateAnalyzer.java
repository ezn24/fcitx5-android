...
public class TextMateAnalyzer extends AsyncIncrementalAnalyzeManager<MyState, Span> implements FoldingHelper, ThemeRegistry.ThemeChangeListener {
    ...
    public TextMateAnalyzer(TextMateLanguage language, IGrammar grammar, LanguageConfiguration languageConfiguration,/* GrammarRegistry grammarRegistry,*/ ThemeRegistry themeRegistry) {
        ...
        // No language-configuration brackets → fall back to the common pairs. Without this,
        // computeBlocks() below pushes a null BracketsProvider into the editor's styleDelegate,
        // wiping any provider installed externally (e.g. via styleDelegate.updateBracketProvider)
        // — leaving foundPair null and matched-bracket highlight invisible.
        if (bracketsProvider == null) {
            bracketsProvider = new OnlineBracketsMatcher(
                new char[] {'(', ')', '[', ']', '{', '}', '<', '>'}, 100000
            );
        }
